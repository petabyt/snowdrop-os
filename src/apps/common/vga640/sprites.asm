...
restore_all_background_rectangles_loop:			; for each sprite
	...
	mov si, di						; DS:SI now points to background rectangle
	mov di, cx						; DI := height (equal to width)
...
save_all_background_rectangles_loop:			; for each sprite
	...
	; now save rectangle
	push di							; [1] save pointer to entry
	push cx							; [2] save loop index
	...
	pop cx							; [2] restore loop index
	pop di							; [1] restore pointer to entry
...
draw_sprite_done_height:
	; here, CX = rendered sprite height
	; here, BX = passed in X coordinate
	mov di, cx					; DI := rendered height
	pop cx						; [3] CX: = rendered width
	mov ax, dx					; AX := Y coordinate
	pop dx						; [2] DX := bitmap total width
	pop si						; [1] DS:SI := passed-in ptr to bitmap
	; all arguments are now setup to draw rectangle
	
	mov bp, sp					; to reference memory
	test byte [ss:bp], SPRITE_FLAG_HFLIP
	jnz draw_sprite_hflipped	; it's horizontally flipped
	; it's not horizontally flipped
