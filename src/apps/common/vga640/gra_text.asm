...
print_centered_calculate_video_offset:
	; here, AX = Y, BX = X, as passed in
	; here, [SP] = options, as passed in
	; here, CL = colour, as passed in
	...
	push ax
	push bx									; [3] save coordinates
	mov bp, sp								; for addressing
common_graphics_text_print_at_loop:
	; here, [BP] = X, [BP+2] = Y
	; here, CL = colour, as passed in
	; here, DX = options, as passed in
...
common_graphics_text_print_at_lf:
	...
	add word [ss:bp+2], COMMON_GRAPHICS_FONT_HEIGHT
...
common_graphics_text_print_at_normal_ascii:
	...
	mov bl, COMMON_GRAPHICS_PIXELS_PER_CHARACTER/8
								; 8 pixels are stored in one byte
...
text_print_at_next_character:
	...
	add word [ss:bp], COMMON_GRAPHICS_FONT_HEIGHT	; move cursor on screen
	jmp common_graphics_text_print_at_loop	; next character
	
common_graphics_text_print_at_done:
	add sp, 4										; [3] cleanup stack
	popa
	ret
...
graphics_text_render_temp_character:
	...
	push cs
	pop ds
	mov si, graphicsTextTempCharacter	; DS:BX now points to temp character

	mov di, COMMON_GRAPHICS_FONT_HEIGHT	; character height
	mov cx, COMMON_GRAPHICS_FONT_WIDTH	; character width
	
	mov DX, COMMON_GRAPHICS_FONT_WIDTH	; bitmap is as wide as the character
...
; Output:
;		AX - printed string width in pixels
common_graphics_text_measure_width_by_count:
	push dx
	mov ax, COMMON_GRAPHICS_FONT_WIDTH
	mul cx					; DX:AX := pixel width of string
	pop dx
	ret
