...
sourceBufferNotSupported:	db 'Error: unsupported compression type', 0
...
usageString: 	db 13, 10
				db 'Example usage:    INFLATE [file=small.txt] [target=large.txt]'
				db 13, 10, 0

outputMessage1:	db 'Decompressed to ', 0
outputMessage2:	db ' bytes. Writing output file...', 13, 10, 0
inputMessage1:	db 'Read in ', 0
inputMessage2:	db ' bytes. Decompressing...', 13, 10, 0
readingMessage:	db 'Reading input file...', 13, 10, 0

fileLoadSegment:	dw 9999h
outputSegment:		dw 9999h					; stores inflated buffer
...
start:
	...
	mov di, 0
	call common_compress_inflate		; [1] CX := output length
	cmp ax, 0
	je not_supported
	
	; report inflated size
	pusha
	push ds
	push cs
	pop ds
...
not_supported:
	push cs
	pop ds
	mov si, sourceBufferNotSupported
	int 80h
	int 95h								; exit
