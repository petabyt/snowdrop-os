;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; The ASMIDE app.
; This is a text editor with integrations with the x86 assembler and debugger
;
...
EDITABLE_CHARACTER_COUNT equ EDITABLE_VRAM_SIZE / 2
...
usageString: 	db 'Source file can also be specified as a parameter:    ASMIDE [file=hello.asm]', 13, 10
				db 13, 10
				db 'Editor commands:', 13, 10
				db '    ESC - exits editor (without confirmation)', 13, 10
				db '     F2 - save file', 13, 10
				db '     F4 - run x86 assembler on file', 13, 10
				db '     F5 - show x86 assembler help', 13, 10
				db '     F6 - run program in x86 debugger', 13, 10
...
fat12FilenameCopy:	times FILE_NAME_MAX_SIZE+1 db 0

initialOverlayString: db ' Snowdrop OS x86 assembler editor', 0
preparingOverlayString: db ' Snowdrop OS x86 assembler editor - preparing...', 0
overlayString1:	db ' File ', 0
overlayString2:	db 'Line ', 0
overlayString3:	db 'F2-Save  F4-Assemble  F5-Help  F6-Debug  ESC-Exit', 0
textFileTitle: 			db 'Text File', 0
enterFileNameString:	db 'Enter file name: ', 0
messagePressAKey:		db 13, 10, 'Press a key to return', 0
...
initialFileLoadAndListingSegment:	dw 0 ; stores file text and then listings
...
outputBytecodeSegment:				dw 0
runAsmCursorPositionBeforeRun:		dw 0
errorCouldNotAllocateRenderedTextSegment:	db 'Error: could not allocate rendered text segment', 13, 10, 0
errorCouldNotAllocateBytecodeSegment:	db 'Error: could not allocate bytecode segment', 13, 10
										db 'Press a key to continue', 0
pressKeyToViewListing:	db 13, 10, 13, 10, 'Press ENTER to view listing and save output, or any other key to return', 0
pressKeyToExit:			db 13, 10, 'Press any key to return', 0
...
pressKeyToSave:		db 13, 10, 13, 10, 'Press ENTER to save output to APP file, or any other key to return', 0
...
start:
	...
	mov word [cs:initialFileLoadAndListingSegment], bx			; store it
...
deallocate_file_load_and_exit:
	...
	mov bx, word [cs:initialFileLoadAndListingSegment]
	int 92h						; deallocate
	int 95h						; exit
...
; it in initialFileLoadAndListingSegment:FILE_LOAD_BUFFER_OFFSET
;
; input:
;		none
; output:
...
load_file:
	...
	push word [cs:initialFileLoadAndListingSegment]
	pop es
...
read_file_name_argument_valid:
	...
	mov di, fat12FilenameCopy	; store a copy
	int 0A6h					; convert 8.3 file name to FAT12 format	
...
delete_line:
	...
	mov byte [ds:MAX_PAGES * EDITABLE_CHARACTER_COUNT], 0
				; add terminator immediately after work area, for the assembler
...
; Runs the x86 assembler on the current file contents.
; If the assembly is successful, it runs the x86 debugger on the
; assembled binary.
;
; input:
;		none
...
debug_asm:
	...
	int 0AAh					; BH := cursor row, BL := cursor column
	mov word [cs:runAsmCursorPositionBeforeRun], bx			; save
	
	int 0A0h							; clear screen
	
	int 91h								; BX := allocated segment
	cmp ax, 0
	jne debug_asm_allocate_bytecode_segment_failed

	mov word [cs:outputBytecodeSegment], bx
	call clear_out_segment
	...
	mov ds, bx
	mov si, 0
	call asm_configure_listing			; we want to generate listing
	
	mov ax, ASM_LOG_CONFIG_TO_VIRTUAL_DISPLAY
	call asm_configure_logging		; we want logging to go to virtual screen
	
	mov ds, word [cs:renderedSegment]
	mov si, RENDERED_BUFFER_OFFSET		; DS:SI := rendered file contents
	mov es, word [cs:outputBytecodeSegment]
	mov di, 0							; ES:DI := output bytecode buffer
	call asm_run

	cmp ax, 0
	je debug_asm_press_key_to_return_and_deallocate	; error during assembly
	mov word [cs:outputSize], cx
	; assembly succeeded, so we can now start the debugger
	...
	; here, CX = binary size in bytes
	; here, ES:DI = pointer to output bytecode buffer
	mov ds, word [cs:initialFileLoadAndListingSegment]
	mov si, 0					; DS:SI := pointer to listing buffer
	call dbgx86_run

	jmp debug_asm_press_key_to_return
	
debug_asm_press_key_to_return_and_deallocate:
	; deallocate bytecode segment
	mov bx, word [cs:outputBytecodeSegment]
	int 92h								; deallocate
debug_asm_press_key_to_return:
	push cs
	pop ds
	mov si, pressKeyToExit
	int 97h
	mov ah, 0
	int 16h
	jmp debug_asm_done
	
debug_asm_allocate_bytecode_segment_failed:
	push cs
	pop ds
	mov si, errorCouldNotAllocateBytecodeSegment
	int 97h
	mov ah, 0
...
debug_asm_done:
	...
	mov bx, word [cs:runAsmCursorPositionBeforeRun]			; restore
	int 9Eh						; position cursor to top left corner
...
run_asm:
	...
	int 91h								; BX := allocated segment
	cmp ax, 0
	jne run_asm_allocate_bytecode_segment_failed
	mov word [cs:outputBytecodeSegment], bx
	call clear_out_segment
	...
	cmp ax, 0
	je run_asm_press_key_to_return			; error during assembly
	mov word [cs:outputSize], cx
	
	; ask user if he wants to view listing
	push cs
	pop ds
	mov si, pressKeyToViewListing
	int 97h
	mov ah, 0
	int 16h							; wait to see if user wants to see listing
	cmp ah, COMMON_SCAN_CODE_ENTER
	jne run_asm_deallocate			; user doesn't want to see listing
	...
	; ask user if he wants to write output bytecode file to disk
	push cs
	pop ds
	mov si, pressKeyToSave
	int 97h
	mov ah, 0
	int 16h							; wait to see if user wants to save output
	cmp ah, COMMON_SCAN_CODE_ENTER
	jne run_asm_deallocate			; user doesn't want to save output
	
	mov si, fat12FilenameCopy				; DS:SI := input file name
	...
	mov es, word [cs:outputBytecodeSegment]
	mov di, 0								; ES:DI := output buffer
	mov cx, word [cs:outputSize]
	int 9Dh									; write file
	
	cmp ax, 1
	je asm_run_max_files_reached
	cmp ax, 2
	je asm_run_disk_full
	cmp ax, 0
	jne asm_run_unknown_error
	; write succeeded	
	jmp run_asm_press_key_to_return
	
asm_run_unknown_error:
	push cs
	pop ds
	mov si, unknownError
	int 97h
...
run_asm_allocate_bytecode_segment_failed:
	push cs
	pop ds
	mov si, errorCouldNotAllocateBytecodeSegment
	int 97h
...
%include "common\assembler\asmx86hl.asm"
%include "common\debugger\dbgx86.asm"

vramDumpBuffer:						; used to convert VRAM to rendered